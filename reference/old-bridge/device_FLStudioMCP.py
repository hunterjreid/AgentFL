...
def OnInit():
    ...
    # PROBE: the hint bar is the one output channel that can be read from
    # outside FL, since the script sandbox blocks sockets and file writes.
    try:
        ui.setHintMsg("CLAUDEPROBE init sysex=" + str(_send_sysex_fn is not None))
    except Exception:
        pass
    if _send_sysex_fn is None:
...
def OnIdle():
    ...
    global _last_heartbeat, _probe_ticks
    now = time.time()
    if now - _last_heartbeat >= HEARTBEAT_INTERVAL:
        _last_heartbeat = now
        # PROBE: runs before the heartbeat so a throw in there is still
        # reported, and repeats every tick so the hint never fades away.
        _probe_ticks += 1
        err = ""
        try:
            _emit_heartbeat()
        except Exception as exc:
            err = " ERR " + type(exc).__name__ + ": " + str(exc)[:60]
        try:
            ui.setHintMsg("CLAUDEPROBE %d sysex=%s%s"
                          % (_probe_ticks, _send_sysex_fn is not None, err))
        except Exception:
            pass
